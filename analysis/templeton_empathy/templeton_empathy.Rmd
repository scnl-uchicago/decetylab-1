...
## Set Up
#### Set up workspace
*Read, reformat, and recode data. Mean center the responses and add Hofstede individualism-collectivism score. Write the output into a clean csv file ("templeton_empathy_clean_data.csv)*

# ```{r, echo = FALSE, results = FALSE,message=FALSE}
# # install.packages("tidyverse", "corrplot", "ggcorrplot", "lme4", "mediation", "here")
# require(tidyverse)
# require(sjstats)
# require(sjPlot)
# require(lmerTest)
...
# }
# 
# # Read, reformat, and recode data
# directory <- "/Users/violet/"
# ifelse (get_os()=="osx", raw_data <- read.csv(paste0(directory,"Desktop/PrimaryCountries_Years1&2_2.csv"), stringsAsFactors = TRUE), raw_data <- read.csv("C:/Users/Violet/desktop/PrimaryCountries_Years1&2_2.csv", stringsAsFactors = TRUE))
# empathy_data <- dplyr::select(raw_data, Subject, Country, Gender_Code, QCAEparent_Aff, QCAEparent_Cog, QCAEparent_tot, QCAEchild_Aff,  QCAEchild_Cog, QCAEchild_tot, Empathy_Pain_RESP, Empathy_Sorry_RESP)
# names(empathy_data) <- tolower(names(empathy_data))
# empathy_data <- dplyr::rename (empathy_data, gender=gender_code)
# empathy_data <- empathy_data %>% mutate(country=dplyr::recode(country, `1`="usa",`2`="canada", `3` = "south_africa", `4` = "turkey", `5` = "china", `6` = "kyrgyzstan", `7` = "jordan"))
# empathy_data <- empathy_data %>% mutate(gender=dplyr::recode(gender, `1`="male",`2`="female"))
# 
# # Create unique ID for each subject based on country and subject ID
# empathy_data <- empathy_data %>% dplyr::mutate(unique_id = paste0(empathy_data$country, "_", empathy_data$subject))
# # Remove any duplicates based on unique ID
# empathy_data <- empathy_data %>% filter(!unique_id%in% empathy_data[(duplicated(empathy_data$unique_id)),]$unique_id)
# 
...
# empathy_data$subject= as.factor(empathy_data$subject)
# 
# # Mean center the responses
# empathy_data <- empathy_data%>%mutate(empathy_pain_resp_c = empathy_pain_resp-mean(empathy_pain_resp, na.rm = TRUE)) # empathy_pain_resp_c is centered
# empathy_data <- empathy_data%>%mutate(empathy_sorry_resp_c = empathy_sorry_resp-mean(empathy_sorry_resp, na.rm = TRUE)) # empathy_sorry_resp_c is centered
# 
# # Add hofstede scores for individualism/collectivism and separate countries as individualist/collectivist based on score
# empathy_data <- empathy_data %>% mutate(individualism = as.numeric((case_when(country=="canada" ~ "80",
#                                                              country=="china" ~ "20",
#                                                              country=="jordan" ~ "30",
#                                                              country=="south_africa" ~ "65",
#                                                              country=="turkey" ~ "37",
#                                                              country=="usa" ~ "91"))))
# empathy_data <- empathy_data %>% mutate(individualism_split = ((case_when(country=="canada" ~ "individualist",
...
# # Add ages
# ifelse (get_os()=="osx", age_data <- read.csv(paste0(directory,"Desktop/vk_age_calculations.csv"), stringsAsFactors = TRUE), age_data <- read.csv("C:/Users/Violet/desktop/vk_age_calculations.csv", stringsAsFactors = TRUE))
# empathy_data <- merge(empathy_data, dplyr::select(age_data, country, subject, vk_age_months, vk_age_years), all.x = TRUE, by = c("country", "subject"))
# empathy_data <- empathy_data %>% dplyr::rename(age_years = vk_age_years, age_months = vk_age_months)
# # Remove any participants with ages outside of 5-12 years
# empathy_data <- empathy_data %>% filter(age_years < 11, age_years > 5)
# write.csv(empathy_data, here("templeton_empathy_clean_data.csv"))
# ```

# Descriptive Statistics
### Note: This file reads the merged and cleaned file from the previous block


``` {r}
empathy_data <- read.csv(here("templeton_empathy_clean_data.csv"))
...
parent_summary <- melt(empathy_data %>%
...
summary(sorry_qcae) # b2 is significant
...
pain_fit <- sem(pain_model, data = pain_data)
