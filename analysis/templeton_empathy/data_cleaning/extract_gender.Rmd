...
clean_additional_data <- clean_additional_data %>% mutate(backup_gender = str_trim(tolower(as.character(Gender)), side = "both"))
clean_additional_data <- clean_additional_data %>% select(unique_id, backup_gender)
clean_additional_data <- clean_additional_data %>% filter(backup_gender != "")
...
output_data <- output_data %>% mutate(gender = ifelse(is.na(gender), backup_gender, gender))
output_data <- output_data %>% select (-backup_gender)

write.csv(combined_data, "templeton_gender_compilation.csv")
